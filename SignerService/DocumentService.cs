...
public class DocumentService {
   ...
   {
      var delay = 100;
      if (configuration["FileTriggerDelay"] != null) {
         int.TryParse(configuration["FileTriggerDelay"], out delay);
      }
      logger.LogDebug("waiting {delay} ms before start file signature", delay);
      Thread.Sleep(delay);
