...
public static class Util
{
    ...
    public static PadesVisualRepresentation2 GetVisualRepresentation(Lacuna.Pki.PKCertificate cert, IConfiguration configuration, Microsoft.Extensions.Logging.ILogger logger)
    {
    ...
    public static ITimestampRequester GetTimestampRequester(IConfiguration configuration)
    {
        var token = configuration.GetValue<string>("RestPkiToken");

        var uri = new Uri(configuration.GetValue<string>("RestPkiUrl"));

        ITimestampRequester request;
        request = new TimestampRequester(uri, token);
