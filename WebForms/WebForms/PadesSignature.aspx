<%@ Page Title="Pades Signature" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="PadesSignature.aspx.cs" Inherits="WebForms.PadesSignature" %>
...
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
	...
	<%--
	UpdatePanel used to refresh only this part of the page. This is used to send the selected certificate's
    encoding to the	code-behind and receiving back the parameters for the signature algorithm computation.
	--%>
	<asp:UpdatePanel runat="server">
		<ContentTemplate>
			...
			<%--
			Hidden button whose click event is fired by the "signature form" javascript upon acquiring the
			selected certificate's encoding. Notice that we cannot use Visible="False" otherwise ASP.NET will
            omit the button altogether from the rendered page, making it impossible to programatically
            "click" it. Notice also that this button is inside the UpdatePanel, triggerring only a partial
            postback.
			--%>
    ...
    later on (see signature-forms.js).
    --%>
    ...
    the button altogether from the rendered page, making it impossible to programatically "click" it. Notice
    also that this button is out of the UpdatePanel, triggering a complete postback.
	--%>
